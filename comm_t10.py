...
def rand_forest(comm, forest):
    ...
    #forest_p['Predicted Close Mean']= forest_p['Predicted Close'].rolling(window=10).mean()
    forest_p['Predicted Close Mean'] = forest_p['Predicted Close'].mean()
    fig_forest = px.line(forest_p, x='Date', y=[comm,'Predicted Close', 'Predicted Close Mean'],color_discrete_map={
                     comm:'green','Predicted Close':'red','Predicted Close Mean':'black'}, width=1000, height=500)             #'Predicted Close', ,'Predicted Close':'red'
